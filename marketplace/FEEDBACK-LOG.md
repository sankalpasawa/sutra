...
- **Closed-in:** plugin v1.14.0 (commit `eff6b97`) → **re-corrected in v1.15.0** (see note below).
- **2026-04-24 follow-up — format correction (v1.15.0):** Founder reviewed the v1.14.0 output and flagged that the summaries described the mechanism ("will delete X", "will push to Y") rather than the outcome in product terms. Three rounds of feedback converged on the right framing: *"what will my world look like after I approve this?"* tied to the user's original task. v1.15.0 ships the reframe + narrows firing to commands that would actually trigger a permission prompt (allow-listed commands skip the hook entirely). LLM is now primary, rules replaced with a single generic fallback line + danger-prefix heuristic. 26/26 sanity tests pass. Fleet receives via next plugin auto-update.
