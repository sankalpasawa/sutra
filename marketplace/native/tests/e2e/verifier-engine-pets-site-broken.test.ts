...
describe('Verifier Engine v1 — pets-site broken-fixture E2E (AC §9.6 — failure surface)', () => {
  it('profile-driven utterance + broken fixtures -> FAIL on Constraint-Honor', async () => {
    ...
    const artifacts = LIVE
      ...
      : readdirSync(resolve(SCENARIO_DIR, 'artifacts.broken-fixture')).map(f => ({
          stage: f.replace(/\.md$/, ''), path: f,
          content: readFileSync(resolve(SCENARIO_DIR, 'artifacts.broken-fixture', f), 'utf8'),
          sha256: 'broken-fixture',
        }))
    const run = {
