...
const MUTABILITY_VALUES = new Set(['mutable', 'immutable']);
const AUTHORITATIVE_STATUS_VALUES = new Set(['authoritative', 'advisory']);
/**
 ...
 */
export function isAssetShape(value) {
    ...
    if (typeof a.schema_ref !== 'string' || a.schema_ref.length === 0)
        return false;
    if (typeof a.locator !== 'string' || a.locator.length === 0)
        return false;
    if (typeof a.version !== 'string')
        return false;
    if (typeof a.retention !== 'string')
        return false;
    // mutability enum
    if (typeof a.mutability !== 'string' || !MUTABILITY_VALUES.has(a.mutability))
        return false;
    ...
    if (typeof a.stable_identity !== 'string' || a.stable_identity.length === 0)
        return false;
    if (!Array.isArray(a.lifecycle_states))
        return false;
 ...
 */
export function isCatalogedAsset(value) {
    if (typeof value !== 'object' || value === null)
        return false;
    const v = value;
    if (typeof v.content_sha256 !== 'string' || !SHA256_HEX_PATTERN.test(v.content_sha256))
        return false;
    if (typeof v.domain_id !== 'string' || !DOMAIN_ID_PATTERN.test(v.domain_id))
        return false;
    if (typeof v.cataloged_at_ms !== 'number' || !Number.isFinite(v.cataloged_at_ms) || v.cataloged_at_ms < 0) {
        return false;
    }
    if (!isAssetShape(v.asset))
        return false;
    ...
    if (!Array.isArray(v.consumes_sha256))
        return false;
    for (const s of v.consumes_sha256) {
        if (typeof s !== 'string' || !SHA256_HEX_PATTERN.test(s))
            return false;
    }
    return true;
}
