 ...
 */
export function telemetrySinkPath(opts) {
 ...
 */
export function readLastSeq(opts) {
 ...
 */
export function resetTelemetryCounter(opts) {
 ...
 */
export function appendTelemetry(event, opts) {
    const path = telemetrySinkPath(opts);
 ...
 */
export function replayTelemetry(opts) {
    const path = telemetrySinkPath(opts);
    if (!existsSync(path))
        return [];
    let raw;
    ...
    for (const line of raw.split('\n')) {
        ...
        try {
            const rec = JSON.parse(line);
            if (typeof rec.seq !== 'number' || !rec.event)
                continue;
            if (rec.seq < fromSeq)
                continue;
            records.push(rec);
 ...
 */
export function readTelemetryRecords(opts) {
    const path = telemetrySinkPath(opts);
    if (!existsSync(path))
        return [];
    let raw;
