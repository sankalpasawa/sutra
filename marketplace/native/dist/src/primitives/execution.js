 ...
 */
import { isValidAgentIdentity, } from '../types/agent-identity.js';
 ...
 */
export function createExecution(spec) {
    ...
    if (spec.state === 'failed' && (reason === null || reason.length === 0)) {
        throw new Error("Execution.failure_reason must be a non-empty string when state='failed' (V2.4 §A12)");
    }
    if (spec.state !== 'failed' && reason !== null) {
        throw new Error(`Execution.failure_reason must be null when state!='failed' (V2.4 §A12); got state='${spec.state}', reason='${reason}'`);
    }
    ...
    if (agentIdentity !== null && !isValidAgentIdentity(agentIdentity)) {
        throw new Error('Execution.agent_identity must be a valid AgentIdentity (M4.2; D-NS-10 namespace prefix per kind)');
    }
    const out = {
        ...spec,
        logs: [...spec.logs],
 ...
 */
export function isValidExecution(e) {
    if (typeof e !== 'object' || e === null)
        return false;
    if (typeof e.id !== 'string' || !E_ID_PATTERN.test(e.id))
        return false;
    if (typeof e.workflow_id !== 'string' || !W_ID_PATTERN.test(e.workflow_id))
        return false;
    if (!VALID_STATES.has(e.state))
        return false;
    if (!Array.isArray(e.logs) || !Array.isArray(e.results))
        return false;
    if (e.parent_exec_id !== null && !E_ID_PATTERN.test(e.parent_exec_id))
        return false;
    if (typeof e.fingerprint !== 'string' || e.fingerprint.length === 0)
        return false;
    if (e.state === 'failed' && (e.failure_reason === null || e.failure_reason.length === 0))
        return false;
    if (e.state !== 'failed' && e.failure_reason !== null)
        return false;
    // M4.2 — agent_identity must be null OR a valid AgentIdentity record.
    if (e.agent_identity !== null && !isValidAgentIdentity(e.agent_identity))
        return false;
    return true;
}
/**
