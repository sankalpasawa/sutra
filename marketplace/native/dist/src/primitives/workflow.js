 ...
 */
import { EXTENSION_REF_PATTERN, } from '../types/extension.js';
...
const VALID_STRINGENCY = new Set([
    'task',
    'process',
    'protocol',
]);
const VALID_OVERRIDE_ACTION = new Set([
    'pause',
    'splice',
    'restart',
    'escalate',
...
const VALID_STEP_ACTION = new Set([
    'spawn_sub_unit',
    'wait',
    'terminate',
    'invoke_host_llm',
...
const VALID_HOST_KIND = new Set(['claude', 'codex']);
const VALID_STEP_FAILURE_ACTION = new Set([
    'rollback',
    'escalate',
    'pause',
    'abort',
...
const VALID_AUTONOMY_LEVEL = new Set([
    'manual',
    'semi',
    'autonomous',
]);
function validateStep(step, idx) {
    ...
    if (hasSkill && hasAction) {
        throw new Error(`Workflow.step_graph[${idx}]: skill_ref XOR action — both provided (mutually exclusive per V2.3 §A11)`);
    }
    if (!hasSkill && !hasAction) {
        throw new Error(`Workflow.step_graph[${idx}]: skill_ref XOR action — must specify exactly one (L2 BOUNDARY)`);
    }
    if (hasAction && !VALID_STEP_ACTION.has(step.action)) {
        throw new Error(`Workflow.step_graph[${idx}].action must be one of spawn_sub_unit|wait|terminate|invoke_host_llm; got "${String(step.action)}"`);
    }
    ...
    if (step.action === 'invoke_host_llm') {
        if (!hasHost) {
            throw new Error(`Workflow.step_graph[${idx}].host is required when action='invoke_host_llm' (M8 Group BB; codex pivot review CHANGE #2 — host-XOR canonical at L2 BOUNDARY)`);
        }
        if (typeof step.host !== 'string' || !VALID_HOST_KIND.has(step.host)) {
            throw new Error(`Workflow.step_graph[${idx}].host must be 'claude' or 'codex' when action='invoke_host_llm'; got "${String(step.host)}"`);
        }
    }
    else {
        if (hasHost) {
            throw new Error(`Workflow.step_graph[${idx}].host is forbidden unless action='invoke_host_llm' (M8 Group BB; codex pivot review CHANGE #2 — host-XOR canonical at L2 BOUNDARY); got host="${String(step.host)}" with action="${String(step.action)}"`);
        }
    }
    ...
    if (!VALID_STEP_FAILURE_ACTION.has(step.on_failure)) {
        throw new Error(`Workflow.step_graph[${idx}].on_failure must be one of rollback|escalate|pause|abort|continue; got "${String(step.on_failure)}"`);
    }
    ...
    if (step.policy_check !== undefined && typeof step.policy_check !== 'boolean') {
        throw new Error(`Workflow.step_graph[${idx}].policy_check must be a boolean when supplied; got "${typeof step.policy_check}"`);
    }
    ...
    if (step.return_contract !== undefined) {
        if (typeof step.return_contract !== 'string' || step.return_contract.length === 0) {
            throw new Error(`Workflow.step_graph[${idx}].return_contract must be a non-empty string when supplied; got "${typeof step.return_contract}"`);
        }
        if (step.action !== 'invoke_host_llm') {
...
function validateExpectsResponseFrom(value) {
    ...
    if (typeof value !== 'string' || value.length === 0) {
        throw new Error('Workflow.expects_response_from must be null or a non-empty BoundaryEndpoint reference string');
    }
}
/**
 ...
 */
export function createWorkflow(spec) {
    ...
    if (!VALID_STRINGENCY.has(spec.stringency)) {
        throw new Error(`Workflow.stringency must be task|process|protocol; got "${String(spec.stringency)}"`);
    }
    ...
    // (isValidWorkflow) for deserialized records.
    {
        ...
        for (let i = 0; i < spec.step_graph.length; i++) {
            ...
            if (seen.has(id)) {
                throw new Error(`Workflow.step_graph[${i}].step_id=${id} duplicates an earlier step (V2 §1 P3 — step_ids MUST be unique within a Workflow; required for L4/T3 coverage soundness)`);
            }
            seen.add(id);
        }
    }
    const onOverride = spec.on_override_action ?? 'escalate';
    if (!VALID_OVERRIDE_ACTION.has(onOverride)) {
        throw new Error(`Workflow.on_override_action must be pause|splice|restart|escalate; got "${String(onOverride)}"`);
    }
    ...
    if (spec.modifies_sutra !== undefined && typeof spec.modifies_sutra !== 'boolean') {
        throw new Error(`Workflow.modifies_sutra must be a boolean; got "${typeof spec.modifies_sutra}"`);
    }
    // V2.3 §A11 — reuse_tag must be boolean (defensive runtime check)
    if (spec.reuse_tag !== undefined && typeof spec.reuse_tag !== 'boolean') {
        throw new Error(`Workflow.reuse_tag must be a boolean; got "${typeof spec.reuse_tag}"`);
    }
    const reuseTag = spec.reuse_tag ?? false;
    const returnContract = spec.return_contract ?? null;
    ...
    if (reuseTag) {
        if (returnContract === null ||
            ...
            throw new Error('Workflow.return_contract is required when reuse_tag=true (V2.3 §A11 — every Skill MUST have a return_contract schema-ref)');
        }
    }
    else {
        ...
        if (returnContract !== null &&
            ...
            throw new Error('Workflow.return_contract must be null OR a non-empty string when reuse_tag=false (constructor/validator alignment per V2 §3 HARD)');
        }
    }
    ...
    if (custodyOwner !== null) {
        if (typeof custodyOwner !== 'string' || !TENANT_ID_PATTERN.test(custodyOwner)) {
            throw new Error(`Workflow.custody_owner must be null or match T-<id> pattern (M4.4; D-NS-11); got "${String(custodyOwner)}"`);
        }
    }
    ...
    const extensionRef = spec.extension_ref ?? null;
    if (extensionRef !== null) {
        if (typeof extensionRef !== 'string' || !EXTENSION_REF_PATTERN.test(extensionRef)) {
            throw new Error(`Workflow.extension_ref must be null or match ext-<id> pattern (M4.5; D4 §7); got "${String(extensionRef)}"`);
        }
    }
    ...
    const autonomyLevel = spec.autonomy_level ?? 'manual';
...
export function isValidWorkflow(w) {
    if (typeof w !== 'object' || w === null)
        return false;
    if (typeof w.id !== 'string' || !W_ID_PATTERN.test(w.id))
        return false;
    if (!Array.isArray(w.step_graph) || w.step_graph.length === 0)
        return false;
    if (!VALID_STRINGENCY.has(w.stringency))
        return false;
    if (!VALID_OVERRIDE_ACTION.has(w.on_override_action))
        return false;
    // V2.1 §A4 — expects_response_from: null OR non-empty string
    if (w.expects_response_from !== null &&
        (typeof w.expects_response_from !== 'string' || w.expects_response_from.length === 0)) {
        return false;
    }
    // V2.4 §A12 — modifies_sutra MUST be boolean (defensive)
    if (typeof w.modifies_sutra !== 'boolean')
        return false;
    // V2.3 §A11 — reuse_tag MUST be boolean (defensive)
    if (typeof w.reuse_tag !== 'boolean')
        return false;
    // V2.3 §A11 HARD — when reuse_tag=true, return_contract MUST be a non-empty string
    if (w.reuse_tag) {
        if (w.return_contract === null ||
    ...
    for (const step of w.step_graph) {
        ...
        const hasAction = typeof step.action === 'string' && step.action.length > 0;
        if (hasSkill === hasAction)
            return false; // both true or both false => invalid
        if (hasAction && !VALID_STEP_ACTION.has(step.action))
            return false;
