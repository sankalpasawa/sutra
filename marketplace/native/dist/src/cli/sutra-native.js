...
function cmdStart(ctx) {
    ...
    if (!result.acquired) {
        if (result.reason === 'lock_held_alive' && result.existing) {
            ...
            ctx.stderr(`sutra-native: already running (pid=${result.existing.pid}, host=${result.existing.host_kind}, started ${ageMin}m ago)\n` +
                `  PID file: ${pidPath}\n` +
...
function formatBanner(hostKind, pidPath) {
    return [
        '┌─ SUTRA-NATIVE ──────────────────────────────────────────────┐',
