...
export class CrossTenantBoundaryError extends Error {
    ...
    operation;
    constructor(input) {
        super(`CrossTenantBoundaryError: ${input.source_tenant} -> ${input.target_tenant} (${input.operation}) — no delegates_to edge granting the operation`);
...
export class TenantIsolation {
     ...
     */
    static assertCrossTenantAllowed(input) {
        if (typeof input !== 'object' || input === null) {
            throw new TypeError('TenantIsolation.assertCrossTenantAllowed: input must be a non-null object');
        }
        ...
        if (typeof source_tenant !== 'string' || source_tenant.length === 0) {
            throw new TypeError('TenantIsolation.assertCrossTenantAllowed: source_tenant must be a non-empty string');
        }
        if (typeof target_tenant !== 'string' || target_tenant.length === 0) {
            throw new TypeError('TenantIsolation.assertCrossTenantAllowed: target_tenant must be a non-empty string');
        }
        if (typeof operation !== 'string' || operation.length === 0) {
            throw new TypeError('TenantIsolation.assertCrossTenantAllowed: operation must be a non-empty string');
        }
     ...
     */
    assertCrossTenantAllowed(input) {
