...
export class SkillEngine {
    ...
    register(skill) {
        if (skill.reuse_tag !== true) {
            throw new Error(`SkillEngine.register: only Skills (reuse_tag=true) can be registered; got reuse_tag=${String(skill.reuse_tag)}`);
        }
        if (skill.return_contract === null ||
            skill.return_contract === undefined ||
            typeof skill.return_contract !== 'string' ||
            skill.return_contract.length === 0) {
            throw new Error(`SkillEngine.register: Skill ${skill.id} missing return_contract (V2 §A11 HARD compliance)`);
        }
        let compiled;
        try {
            ...
            const parsed = JSON.parse(skill.return_contract);
            compiled = this.ajv.compile(parsed);
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
            throw new Error(`SkillEngine.register: return_contract for ${skill.id} is not valid JSON Schema: ${msg}`);
        }
     ...
     */
    validateOutputs(skill_ref, outputs) {
