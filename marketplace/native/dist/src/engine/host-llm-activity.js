...
export class HostUnavailableError extends Error {
    host;
    constructor(host) {
        super(`Host LLM '${host}' is unavailable. Install via: ` +
            (host === 'claude'
 ...
 */
function defaultInvokeHostLLMF12Probe() {
    try {
        const wfMod = __require('@temporalio/workflow');
...
let invokeHostLLMF12Probe = defaultInvokeHostLLMF12Probe;
/**
 ...
 */
export function __setInvokeHostLLMF12ProbeForTest(probe) {
    invokeHostLLMF12Probe = probe;
}
export function __resetInvokeHostLLMF12ProbeForTest() {
...
export function __setHostAvailabilityForTest(map) {
    availabilityOverride = map;
}
export function __resetHostAvailabilityForTest() {
    availabilityOverride = null;
}
function getAvailability(host) {
...
let execFileSyncStub = null;
export function __setExecFileSyncStubForTest(stub) {
    execFileSyncStub = stub;
}
export function __resetExecFileSyncStubForTest() {
 ...
 * underlying response.
 */
function deriveInvocationId(prompt, host_kind, host_version, workflow_run_seq) {
 ...
 */
export function invokeHostLLM(args) {
    ...
    if (invokeHostLLMF12Probe()) {
        throw new Error('F-12: opa/host LLM invocation in Workflow context. ' +
    ...
    const exec = execFileSyncStub ?? execFileSync;
    let response;
    let exit_code = 0;
    try {
        if (args.host === 'claude') {
        ...
        throw new Error(`Host LLM '${args.host}' invocation failed (exit ${exit_code}): ${err.message ?? 'unknown error'}`);
    }
    const invocation_id = deriveInvocationId(args.prompt, args.host, avail.version, args.workflow_run_seq);
    return {
        response,
        host_kind: args.host,
 ...
 */
export const hostLLMActivity = asActivity(async (args) => {
    return invokeHostLLM(args);
});
//# sourceMappingURL=host-llm-activity.js.map