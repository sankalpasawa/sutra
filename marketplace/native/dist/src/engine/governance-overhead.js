...
export class GovernanceOverhead {
     ...
     */
    startTurn(turn_id, tokens_total_estimate) {
        if (!Number.isFinite(tokens_total_estimate) || tokens_total_estimate < 0) {
            throw new RangeError(`GovernanceOverhead.startTurn: tokens_total_estimate must be a non-negative finite number (got ${tokens_total_estimate})`);
        }
     ...
     * pin this.
     */
    track(turn_id, category, tokens) {
        const state = this.turns.get(turn_id);
        if (state === undefined) {
            throw new TurnNotStartedError(turn_id);
        }
        if (!Number.isFinite(tokens) || tokens < 0) {
            throw new RangeError(`GovernanceOverhead.track: tokens must be a non-negative finite number (got ${tokens})`);
        }
    ...
    buildReport(turn_id, state) {
        let governance = 0;
        for (const v of state.per_category.values())
            governance += v;
...
//# sourceMappingURL=governance-overhead.js.map