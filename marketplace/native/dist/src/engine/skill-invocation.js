 ...
 */
function synthesizeChildExecutionId(parent_step_id, skill_ref) {
 ...
 */
function isSkillOutputDataRef(value) {
    if (typeof value !== 'object' || value === null)
        return false;
    const v = value;
    return (v.kind === 'skill-output' &&
        typeof v.locator === 'string' &&
        v.locator.startsWith('inline:'));
}
/**
 ...
 */
function extractTerminalPayload(child_result) {
 ...
 */
function buildSkillOutputDataRef(payload, return_contract) {
    return {
        kind: 'skill-output',
 ...
 */
function stripFailurePolicyPrefix(reason) {
 ...
 */
export async function invokeSkill(parentStep, context) {
    ...
    const validated_dataref = buildSkillOutputDataRef(validated_payload, resolved.return_contract);
 ...
 */
async function emitSkillEvent(context, kind, step_id, skill_ref, extra) {
