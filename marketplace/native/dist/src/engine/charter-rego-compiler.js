...
export class BuiltinNotAllowedError extends Error {
    builtin_name;
    charter_id;
    constructor(builtin_name, charter_id) {
        super(`Charter ${charter_id} compiles to Rego using forbidden builtin: ${builtin_name}`);
 ...
 */
const FORBIDDEN_BUILTIN_PATTERNS = [
    /\bhttp\./,
    /\bnet\./,
    /\btime\./,
    /\brand\./,
 ...
 */
export function checkBuiltinAllowlist(rego_source, charter_id) {
 ...
 */
function sanitizeIdent(raw, fallback) {
...
function regoPackageLine(policy_id) {
 ...
 */
function regoAllowRules(constraints) {
    const rules = [];
 ...
 */
function regoObligationDenyBlocks(obligations) {
    const blocks = [];
    let index = 0;
 ...
 */
export function compileCharter(charter) {
    ...
    if (obligation_blocks.length > 0) {
        lines.push('');
        for (const block of obligation_blocks)
            lines.push(block);
    }
...
//# sourceMappingURL=charter-rego-compiler.js.map