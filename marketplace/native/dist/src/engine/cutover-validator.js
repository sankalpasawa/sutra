 ...
 */
export function validateCutoverContract(contract) {
    const errors = [];
    ...
    if (c.source_engine === c.target_engine) {
        errors.push(`source_engine === target_engine ("${c.source_engine}") — cutover would be a no-op`);
    }
    ...
    for (const inv of c.behavior_invariants) {
        ...
        if (seenInvariants.has(trimmed)) {
            errors.push(`behavior_invariants[]: duplicate entry "${trimmed}" — each invariant must be unique`);
        }
        seenInvariants.add(trimmed);
    }
    // (4) canary_window must parse as a recognized duration.
    if (!isParseableDuration(c.canary_window)) {
        errors.push(`canary_window: "${c.canary_window}" not a recognized duration — expected ISO-8601 "PT72H"/"P7D", short-form "7d"/"60s", or integer seconds`);
    }
 ...
 * Pure — no I/O.
 */
export function isParseableDuration(input) {
    const s = input.trim();
    if (s.length === 0)
        return false;
    // Plain integer
    ...
    if (iso)
        return Number(iso[1]) > 0;
    // Short form
    const short = /^(\d+)([smhd])$/.exec(s);
    if (short)
        return Number(short[1]) > 0;
    return false;
}
/**
 * Type-narrowing predicate. Convenience for callers.
 */
export function isValidatedCutoverContract(v) {
    const r = validateCutoverContract(v);
    return r.valid;
}
//# sourceMappingURL=cutover-validator.js.map