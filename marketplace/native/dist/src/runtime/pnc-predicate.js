 ...
 */
export function isPNCPredicate(value) {
 ...
 */
export function evaluatePNC(predicate, ctx, registry) {
    switch (predicate.type) {
        ...
        case 'or': {
            if (predicate.clauses.length === 0)
                return { verdict: false, reason: 'or:empty_clauses' };
            const reasons = [];
            for (const c of predicate.clauses) {
                const r = evaluatePNC(c, ctx, registry);
                if (r.verdict)
 ...
 */
export const BASELINE_PREDICATE_REGISTRY = new Map([
    ['always_true', () => true],
    ['always_false', () => false],
]);
//# sourceMappingURL=pnc-predicate.js.map