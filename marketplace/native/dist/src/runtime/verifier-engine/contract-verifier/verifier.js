...
import { runTier4Rubric } from './layer-rubric.js';
export async function verify(contract, run, opts) {
    const t0 = Date.now();
    ...
    if (tier1.some(r => r.status === 'FAIL')) {
        ...
        return {
            ...
            results: tier1, summary, verdict: 'FAIL',
        };
    }
    const results = [
        ...tier1,
    ...
    const hardFail = results.some(r => (r.tier === 'schema' || r.tier === 'artifact' || r.tier === 'behavior') && r.status === 'FAIL');
    const verdict = hardFail ? 'FAIL' : (summary.fail > 0 ? 'PARTIAL' : 'PASS');
    return {
        run_id: run.run_id, scenario_id: run.scenario_id,
        duration_ms: Date.now() - t0,
