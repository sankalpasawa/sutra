 ...
 */
import { unwrapJson } from './extractor.js';
...
export async function validateAndRetry(prompt, llm, max_retries) {
    let last_error = '';
    for (let attempt = 0; attempt <= max_retries; attempt++) {
        ...
        try {
            ...
            const conf = mutable.confidence;
            const flags = mutable.ambiguity_flags;
...
function validateContractShape(c) {
    ...
    if (typeof obj.clarification_required !== 'boolean')
        throw new Error('clarification_required must be boolean');
    for (const f of ['hard_constraints', 'non_goals', 'acceptance_examples', 'evidence_required', 'ambiguity_flags']) {
        if (!Array.isArray(obj[f]))
            throw new Error(`${f} must be array`);
    }
    if (typeof obj.execution_preferences !== 'object' ||
    ...
    for (const f of ['situation', 'motivation', 'outcome', 'beneficiary']) {
        if (typeof obj[f] !== 'string')
            throw new Error(`${f} must be string`);
    }
    ...
    for (const [i, ex] of obj.acceptance_examples.entries()) {
        if (typeof ex !== 'object' || ex === null)
            throw new Error(`acceptance_examples[${i}] must be object`);
        const e = ex;
        for (const k of ['given', 'when', 'then']) {
            if (typeof e[k] !== 'string')
                throw new Error(`acceptance_examples[${i}].${k} must be string`);
        }
    }
    // evidence_required[i] shape: { stage, field } both strings.
    for (const [i, ev] of obj.evidence_required.entries()) {
        if (typeof ev !== 'object' || ev === null)
            throw new Error(`evidence_required[${i}] must be object`);
        const e = ev;
        for (const k of ['stage', 'field']) {
            if (typeof e[k] !== 'string')
                throw new Error(`evidence_required[${i}].${k} must be string`);
        }
    }
    // ambiguity_flags elements must be strings.
    for (const [i, f] of obj.ambiguity_flags.entries()) {
        if (typeof f !== 'string')
            throw new Error(`ambiguity_flags[${i}] must be string`);
    }
}
