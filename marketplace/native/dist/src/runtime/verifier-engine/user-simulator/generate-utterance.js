...
export function generateUtterance(profile, scenario) {
    if (profile.id !== scenario.profile_id) {
        throw new Error(`profile_id mismatch: profile=${profile.id} scenario=${scenario.profile_id}`);
    }
    const missing = missingSubstitutions(profile, scenario);
    if (missing.length > 0) {
        throw new Error(`missing substitution(s): ${missing.join(', ')}`);
    }
    return profile.utterance_template.replace(PLACEHOLDER, (_, key) => scenario.goal_seed[key] ?? '');
}
export function missingSubstitutions(profile, scenario) {
...
//# sourceMappingURL=generate-utterance.js.map