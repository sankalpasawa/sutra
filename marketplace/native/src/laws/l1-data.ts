...
export const l1Data = {
  ...
  shouldPromoteToAsset(ref: DataRef | Asset | unknown): boolean {
    ...
    if (!('stable_identity' in r)) return false
    if (typeof r.stable_identity !== 'string') return false
    if (r.stable_identity.length === 0) return false

    // V2 §3 L1 mechanization step 2: lifecycle_states.length > 1.
    if (!('lifecycle_states' in r)) return false
    if (!Array.isArray(r.lifecycle_states)) return false
    if (r.lifecycle_states.length <= 1) return false
