 ...
 */
export const CutoverContractSchema = z
  .object({
    source_engine: z.string().min(1),
    target_engine: z.string().min(1),
    behavior_invariants: z.array(z.string().min(1)).min(1),
    rollback_gate: z.string().min(1),
    canary_window: z.string().min(1),
  })
  .nullable()
 ...
 */
export function createCutoverContract(input: CutoverContract): CutoverContract {
  return CutoverContractSchema.parse(input)
}
 ...
 */
export function isValidCutoverContract(v: unknown): v is CutoverContract {
  return CutoverContractSchema.safeParse(v).success
}
