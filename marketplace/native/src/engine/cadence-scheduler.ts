...
export type CadenceSpec =
  ...
  | { kind: 'every_day_at'; hour_utc: number; minute_utc: number }
...
/**
 * Internal record per registered cadence. Held in the scheduler's Map.
 * `next_fire_at` is recomputed after every fire (period kinds) or on
 * every tick (every_day_at kind) so the registration record stays a
 * single source of truth.
 */
interface RegisteredCadence {
  readonly handle: CadenceHandle
  readonly spec: CadenceSpec
  readonly callback: CadenceCallback
  /** ms — clock-derived. -Infinity at registration BEFORE first start(). */
  next_fire_at: number
  /**
   * Set on register(); used as the floor for the first-fire computation
   * so a cadence registered at t=10 doesn't fire at t=11 with `Math.max`
   * skew.
   */
  registered_at: number
  /**
   * ms — actual fire timestamp recorded by tick(). Optional; populated
   * after the first fire. Used by `getJitterMs(handle)` so observability
   * + the I-12 property test can read the most-recent jitter without
   * external accounting.
   */
  last_actual_fire_at?: number
  last_scheduled_fire_at?: number
}
...
export class CadenceScheduler {
  private readonly registry: Map<CadenceHandle, RegisteredCadence> = new Map()
  ...
  register(spec: CadenceSpec, callback: CadenceCallback): CadenceHandle {
    ...
    const handle: CadenceHandle = `cad-${++this.nextHandleSeq}`
   ...
   */
  async tick(now?: number): Promise<TickReport> {
    if (!this.started) {
      return { fires: [], errors: [] }
    }
    const t = typeof now === 'number' && Number.isFinite(now) ? now : this.clock()

    const fires: TickFireEntry[] = []
    const errors: TickErrorEntry[] = []
    ...
    for (const handle of handles) {
      ...
      if (cad === undefined) continue // unregistered mid-tick
      if (cad.next_fire_at > t) continue
      // cron handles never fire at v1.0 — their next_fire_at is +Infinity.
      if (!Number.isFinite(cad.next_fire_at)) continue
...
function validateSpec(spec: CadenceSpec): void {
  ...
  switch (spec.kind) {
    ...
    case 'every_n_hours': {
      if (
        typeof spec.n !== 'number' ||
        !Number.isInteger(spec.n) ||
        spec.n <= 0
      ) {
        throw new RangeError(
          `CadenceScheduler.register: ${spec.kind}.n must be a positive integer (got ${String(spec.n)})`,
        )
      }
      return
    }
    case 'every_day_at': {
      if (
        typeof spec.hour_utc !== 'number' ||
        !Number.isInteger(spec.hour_utc) ||
        spec.hour_utc < 0 ||
        spec.hour_utc > 23
      ) {
        throw new RangeError(
          `CadenceScheduler.register: every_day_at.hour_utc must be an integer in [0, 23] (got ${String(spec.hour_utc)})`,
        )
      }
      if (
        typeof spec.minute_utc !== 'number' ||
        !Number.isInteger(spec.minute_utc) ||
        spec.minute_utc < 0 ||
        spec.minute_utc > 59
      ) {
        throw new RangeError(
          `CadenceScheduler.register: every_day_at.minute_utc must be an integer in [0, 59] (got ${String(spec.minute_utc)})`,
        )
      }
      return
    }
    case 'cron': {
      if (typeof spec.expression !== 'string' || spec.expression.length === 0) {
        throw new TypeError(
          'CadenceScheduler.register: cron.expression must be a non-empty string',
        )
      }
      return
    }
    default: {
      ...
      throw new TypeError(
        `CadenceScheduler.register: unknown spec.kind (got ${String((_exhaustive as { kind?: string }).kind)})`,
      )
    }
  }
}
...
function periodMs(spec: CadenceSpec): number {
  switch (spec.kind) {
    case 'every_n_minutes':
 ...
 */
function computeFirstFireAt(spec: CadenceSpec, now: number): number {
  ...
  if (spec.kind === 'cron') return Number.POSITIVE_INFINITY
 ...
 */
function computeNextFireAt(spec: CadenceSpec, scheduled: number, _actual: number): number {
  if (spec.kind === 'cron') return Number.POSITIVE_INFINITY
