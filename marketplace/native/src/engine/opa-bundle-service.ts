 ...
 */
export class OPABundleService {
  private readonly bundles: Map<string, Map<string, CompiledPolicy>> = new Map()
  private readonly latest: Map<string, string> = new Map()
  ...
  register(policy: CompiledPolicy): void {
    let versions = this.bundles.get(policy.policy_id)
    if (versions === undefined) {
      versions = new Map()
      this.bundles.set(policy.policy_id, versions)
    }
    versions.set(policy.policy_version, policy)
    this.latest.set(policy.policy_id, policy.policy_version)
  }
  ...
  unregister(policy_id: string): void {
    this.bundles.delete(policy_id)
    this.latest.delete(policy_id)
  }
  ...
  get(policy_id: string, policy_version?: string): CompiledPolicy | null {
    const versions = this.bundles.get(policy_id)
    if (versions === undefined) return null
    const v = policy_version ?? this.latest.get(policy_id)
  ...
  list_versions(policy_id: string): string[] {
    const versions = this.bundles.get(policy_id)
    return versions === undefined ? [] : [...versions.keys()]
  }
}
