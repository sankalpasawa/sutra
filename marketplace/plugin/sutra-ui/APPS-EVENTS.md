...
| app.published | (Publish program P3) approval of an `app.publish` proposal ran export, version bump, signing and staged the entry and tarball under `~/.sutra-ui/publish/`; `actor` is `proposal:<id>` so the approval trail is on the row | `publish_version`, `key_id`, `registry` |
| app.registry_refreshed | (Publish program P4) one row per `POST /api/modules/registry/refresh`, never per read; `app_id` is `*` (a registry-wide fact, not one app's) | `checked`, `updates`, `incompatible`, `registries` |
