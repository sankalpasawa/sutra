...
4c. SUTRA_SHADOW_SMOKE=1 .venv/bin/python -m pytest -q -s test_shadow_smoke_cycle.py   # OPT-IN, not part of the gate: one REAL watch -> say -> verify cycle with the real claude binary against the live ~/.sutra-ui/shadow; leaves a say + done action row for a real mission. Two Claude turns.
