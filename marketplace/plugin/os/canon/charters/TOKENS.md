<!-- MIRROR (read-only distribution copy). Canonical: sutra/os/charters/TOKENS.md. -->
# Charter: Tokens
...
| 4 | Extend `holding/hooks/dispatcher-stop.sh` to emit `task_end` events with governance-vs-work categorization | KR1 | Engineering | 2026-04-24 | TODO — next chunk. **Design decision deferred (2026-04-23)**: Option A = extend holding/hooks/dispatcher-stop.sh (L1 staging, +1 promotion debt, fast); Option B = build new plugin L0 hook `sutra/marketplace/plugin/hooks/token-task-end.sh` + bump plugin v1.12→v1.13 (~40 min, fleet-wide on publish, 0 debt, consistent w/ today's 4 L0 promotions). Recommendation: **Option B** per PROTO-021 "can this be L0?" question + today's pattern. Hard part either option = governance-vs-work token split — ship thin v1 with heuristic (Input-Routing+Depth block lines × 4.5 tokens/line, proxy=true), refine in W2 with real data. Decision blocked on founder sign-off at resumption. |
