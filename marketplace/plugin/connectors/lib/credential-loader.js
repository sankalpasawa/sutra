 ...
 */
import { appendFileSync, closeSync, existsSync, fsyncSync, lstatSync, mkdirSync, openSync, readFileSync, renameSync, writeSync, } from 'node:fs';
...
export class CredentialLoader {
    ...
    async load(connector_name) {
        ...
        if (existsSync(agePath)) {
            const decryptOpts = this.#signal !== undefined ? { signal: this.#signal } : {};
    ...
    // -------------------------------------------------------------------------
    async save(connector_name, bundle) {
        if (typeof connector_name !== 'string' || connector_name.length === 0) {
        ...
        const encryptOpts = this.#signal !== undefined ? { signal: this.#signal } : {};
    ...
    //     - fsync, atomic rename
    // -------------------------------------------------------------------------
    #writeJsonShadow(target, plaintext) {
    ...
    //   credential reads.
    // -------------------------------------------------------------------------
    #emitMigrationBeacon(connector, jsonPath) {
        const event = {
            ts: Date.now(),
