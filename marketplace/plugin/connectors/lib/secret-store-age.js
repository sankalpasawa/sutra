 ...
 */
import { closeSync, existsSync, fsyncSync, lstatSync, mkdirSync, openSync, renameSync, unlinkSync, writeSync, } from 'node:fs';
import { spawn } from 'node:child_process';
import { dirname } from 'node:path';
import { constants as fsConstants } from 'node:fs';
import { SecretStoreSafetyError, SecretStoreTimeoutError, SecretStoreDecryptError, AbortError, } from './errors.js';
...
export class SecretStoreAge {
    ...
    // --------------------------------------------------------------------------
    async encrypt(target, plaintext, opts = {}) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const timer = setTimeout(() => {
                settle(() => {
                    ...
                    reject(new SecretStoreTimeoutError(`age -d exceeded ${timeoutMs}ms for ${target}`));
                });
            }, timeoutMs);
            timer.unref();
            const onAbort = () => {
            ...
            proc.on('close', (code) => {
                settle(() => {
                    ...
                    if (signal)
                        signal.removeEventListener('abort', onAbort);
                    if (code !== 0) {
                        reject(new SecretStoreDecryptError(`age -d exited ${code}: ${stderrBuf.trim()}`));
                        return;
                    }
    ...
    // --------------------------------------------------------------------------
    async #runAgeEncrypt(fd, plaintext, opts) {
        const signal = opts.signal;
        const timeoutMs = opts.timeoutMs ?? DEFAULT_TIMEOUT_MS;
        ...
        return new Promise((resolve, reject) => {
            ...
            const timer = setTimeout(() => {
                settle(() => {
                    ...
                    if (signal)
                        signal.removeEventListener('abort', onAbort);
                    reject(new SecretStoreTimeoutError(`age -e exceeded ${timeoutMs}ms`));
                });
            }, timeoutMs);
            timer.unref();
            const onAbort = () => {
            ...
            proc.on('close', (code) => {
                settle(() => {
                    ...
                    if (signal)
                        signal.removeEventListener('abort', onAbort);
                    if (code !== 0) {
                        reject(new SecretStoreDecryptError(`age -e exited ${code}: ${stderrBuf.trim()}`));
                        return;
                    }
                    resolve();
                });
