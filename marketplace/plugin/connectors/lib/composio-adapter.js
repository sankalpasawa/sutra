...
export const FORBIDDEN_COMPOSIO_APIS = [
    'plan',
    'discover',
    'workbench',
    'session-memory',
...
export class ComposioAdapter {
    ...
    constructor(client) {
        ...
        if (typeof client.authenticate !== 'function') {
            throw new Error("ComposioAdapter: client missing required method 'authenticate' " +
                '(narrow surface contract — LLD §2.7)');
        }
        if (typeof client.executeTool !== 'function') {
            throw new Error("ComposioAdapter: client missing required method 'executeTool' " +
                '(narrow surface contract — LLD §2.7)');
        }
        if (typeof client.isAuthenticated !== 'function') {
            throw new Error("ComposioAdapter: client missing required method 'isAuthenticated' " +
                '(narrow surface contract — LLD §2.7)');
        }
        this.#client = client;
    }
     ...
     * is dumb on purpose.
     */
    async call(toolkit, tool, args, opts) {
