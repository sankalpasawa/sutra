...
export { ConnectorError, ManifestError, PolicyDeniedError, StalePolicyError, ApprovalRequiredError, ApprovalTokenExpiredError, ForbiddenComposioApiError, IdempotencyKeyRequiredError, PayloadTooLargeError, CredentialNotFoundError, SecretStoreSafetyError, SecretStoreTimeoutError, SecretStoreDecryptError, AbortError, } from './errors.js';
...
import { StalePolicyError, ForbiddenComposioApiError, IdempotencyKeyRequiredError, PayloadTooLargeError, } from './errors.js';
...
export class ConnectorRouter {
    ...
    constructor(opts) {
        ...
        if (opts.mode !== 'legacy' && opts.mode !== 'native-compat') {
            throw new Error(`ConnectorRouter: mode must be 'legacy' | 'native-compat' (got: ${String(opts.mode)})`);
        }
        if (opts.mode === 'native-compat' && !opts.credentialLoader) {
            throw new Error('ConnectorRouter: mode=native-compat requires credentialLoader');
        }
    ...
    async call(ctx) {
        ...
        if (this.#mode === 'native-compat') {
            if (typeof ctx.idempotency_key !== 'string' ||
                ctx.idempotency_key.length === 0) {
                throw new IdempotencyKeyRequiredError('ConnectorRouter mode=native-compat requires ctx.idempotency_key');
            }
            if (!(ctx.signal instanceof AbortSignal)) {
                throw new IdempotencyKeyRequiredError('ConnectorRouter mode=native-compat requires ctx.signal: AbortSignal');
            }
        }
        // ----------------------------------------------------------------------
        ...
        let consumedToken;
        if (typeof ctx.approvalToken === 'string' &&
        ...
        try {
            if (this.#mode === 'native-compat') {
                ...
                rawResult = await policy.execute(() => this.#adapter.call(manifest.composioToolkit, toolName, ctx.args, adapterOpts), ctx.signal);
            }
            else {
                rawResult = await this.#adapter.call(manifest.composioToolkit, toolName, ctx.args);
            }
        }
        catch (err) {
        ...
        // ----------------------------------------------------------------------
        const successOutcome = approvedAfterGate
            ? 'approved-after-gate'
            : 'allowed';
        await this.#auditSafe({
            ts: ctx.ts,
 ...
 */
function extractConnectorSegment(capability) {
 ...
 */
function extractToolName(capability) {
 ...
 */
function applyRedactPaths(result, redactPaths) {
