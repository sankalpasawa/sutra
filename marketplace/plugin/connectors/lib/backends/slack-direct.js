...
export class SlackDirectClient {
    #loadCredential;
    #saveCredential;
    constructor(loadCredential, saveCredential) {
     ...
     * this client.
     */
    async authenticate(_toolkit, oauthToken) {
     ...
     */
    async executeTool(_toolkit, tool, args, opts) {
     ...
     * `ok=false`.
     */
    async isAuthenticated(_toolkit) {
        const cred = await this.#loadCredential();
        if (!cred)
            return false;
        try {
    ...
    // --------------------------------------------------------------------
    async #post(method, token, body, signal) {
