...
): { connector: string; action: string; resource: string | undefined } {
 ...
 */
function tierEntryCovers(entry: Capability, capability: Capability): boolean {
  if (entry === capability) {
    return true;
  }
...
export function isOverbroadCapability(capability: Capability): boolean {
  if (!capability) {
    return false;
  }
  const { connector, action } = splitCapability(capability);
