...
const COSTS = ['free', 'cents', 'dollars'];
...
export function parseManifest(yamlText) {
    ...
    if (raw === null || raw === undefined || typeof raw !== 'object' || Array.isArray(raw)) {
        throw new ManifestError('malformed YAML: top-level value must be a mapping', 'malformed');
    }
    // schemaVersion is checked first so version-skew is the loudest failure.
    const obj = raw;
    if (!('schemaVersion' in obj)) {
        throw new ManifestError("schemaVersion is required and must be '1'", 'schema-version');
    }
    if (obj.schemaVersion !== '1') {
        throw new ManifestError(`schemaVersion must be '1' (got ${JSON.stringify(obj.schemaVersion)})`, 'schema-version');
    }
...
export function validateManifest(m) {
    if (m === null || m === undefined || typeof m !== 'object') {
        throw new ManifestError('manifest must be an object', 'missing-field');
    }
    // schemaVersion (validated by parseManifest too; redundant for direct callers)
    if (m.schemaVersion !== '1') {
        throw new ManifestError("schemaVersion must be '1'", 'schema-version');
    }
    // name
    ...
    if (typeof m.composioToolkit !== 'string' || m.composioToolkit.length === 0) {
        throw new ManifestError('composioToolkit must be a non-empty string', 'missing-field');
    }
    ...
    if (integration !== undefined && integration !== null) {
        if (typeof integration !== 'object' || Array.isArray(integration)) {
            throw new ManifestError('integration must be an object with kind and target', 'missing-field');
        }
        const integ = integration;
        if (typeof integ.kind !== 'string' ||
            !['composio', 'direct'].includes(integ.kind)) {
            throw new ManifestError('integration.kind must be "composio" or "direct"', 'invalid-capability');
        }
        if (typeof integ.target !== 'string' || integ.target.length === 0) {
            throw new ManifestError('integration.target must be a non-empty string', 'missing-field');
        }
    }
    // capabilities — non-empty array
    if (!Array.isArray(m.capabilities) || m.capabilities.length === 0) {
        throw new ManifestError('capabilities must be a non-empty array', 'missing-field');
    }
    ...
    if (m.tierAccess === null ||
        m.tierAccess === undefined ||
        typeof m.tierAccess !== 'object' ||
        Array.isArray(m.tierAccess)) {
        throw new ManifestError('tierAccess must be an object with T1, T2, T3, T4 keys', 'missing-field');
    }
    for (const tier of TIERS) {
        if (!(tier in m.tierAccess)) {
            throw new ManifestError(`tierAccess.${tier} is required`, 'missing-field');
        }
        const list = m.tierAccess[tier];
        if (!Array.isArray(list)) {
            throw new ManifestError(`tierAccess.${tier} must be an array of capability ids`, 'missing-field');
        }
        for (const ref of list) {
            if (typeof ref !== 'string' || ref.length === 0) {
                throw new ManifestError(`tierAccess.${tier} entries must be non-empty capability id strings`, 'invalid-capability');
            }
            // Reject overbroad references at tier level too.
            if (isOverbroad(ref)) {
                throw new ManifestError(`tierAccess.${tier} references overbroad capability ${JSON.stringify(ref)}`, 'overbroad-capability');
            }
            if (!seenIds.has(ref) && !matchesKnownCapabilityPrefix(ref, seenIds)) {
                throw new ManifestError(`tierAccess.${tier} references unknown capability ${JSON.stringify(ref)}`, 'invalid-capability');
            }
        }
    }
    ...
    for (const f of m.auditFields) {
        if (typeof f !== 'string') {
            throw new ManifestError('auditFields entries must be strings', 'missing-field');
        }
    }
    ...
    for (const p of m.redactPaths) {
        if (typeof p !== 'string') {
            throw new ManifestError('redactPaths entries must be strings', 'missing-field');
        }
    }
}
// -----------------------------------------------------------------------------
...
function validateCapability(cap) {
    if (cap === null || cap === undefined || typeof cap !== 'object') {
        throw new ManifestError('capability entry must be an object', 'invalid-capability');
    }
    // id — non-empty string
    if (typeof cap.id !== 'string' || cap.id.length === 0) {
        throw new ManifestError('capability.id must be a non-empty string', 'invalid-capability');
    }
    // overbroad check
    if (isOverbroad(cap.id)) {
        throw new ManifestError(`capability.id ${JSON.stringify(cap.id)} is overbroad — connector and action segments must be concrete (not '*')`, 'overbroad-capability');
    }
    // action
    if (!ACTIONS.includes(cap.action)) {
        throw new ManifestError(`capability.action must be one of ${ACTIONS.join('|')} (got ${JSON.stringify(cap.action)})`, 'invalid-capability');
    }
    // resourcePattern
    if (typeof cap.resourcePattern !== 'string' || cap.resourcePattern.length === 0) {
        throw new ManifestError('capability.resourcePattern must be a non-empty string', 'invalid-capability');
    }
    // minDepth
    if (!DEPTHS.includes(cap.minDepth)) {
        throw new ManifestError(`capability.minDepth must be one of 1..5 (got ${JSON.stringify(cap.minDepth)})`, 'invalid-capability');
    }
    // approvalRequired
    if (typeof cap.approvalRequired !== 'boolean') {
        throw new ManifestError('capability.approvalRequired must be boolean', 'invalid-capability');
    }
    // costEstimate
    if (!COSTS.includes(cap.costEstimate)) {
        throw new ManifestError(`capability.costEstimate must be one of ${COSTS.join('|')} (got ${JSON.stringify(cap.costEstimate)})`, 'invalid-capability');
    }
    ...
    if (idResource !== null && !globMatch(cap.resourcePattern, idResource)) {
        throw new ManifestError(`capability.id resource ${JSON.stringify(idResource)} does not match resourcePattern ${JSON.stringify(cap.resourcePattern)}`, 'invalid-capability');
    }
}
/**
 ...
 */
function matchesKnownCapabilityPrefix(ref, knownIds) {
