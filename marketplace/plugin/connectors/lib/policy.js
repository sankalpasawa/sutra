 ...
 */
export function issueApproval(ctx) {
 ...
 */
export function isApprovalValidFor(token, ctx) {
 ...
 */
export function consumeApproval(token, ctx) {
    if (typeof token !== 'string' || token.length === 0)
        return false;
    const binding = ISSUED_TOKENS.get(token);
    if (binding === undefined)
    ...
    if (ctx === undefined)
        return false;
    if (!bindingMatches(binding, ctx))
        return false;
    ISSUED_TOKENS.delete(token);
    return true;
}
function bindingMatches(binding, ctx) {
    if (binding.clientId !== ctx.clientId)
        return false;
    if (binding.sessionId !== ctx.sessionId)
        return false;
    if (binding.capability !== ctx.capability)
        return false;
...
// ---------------------------------------------------------------------------
export function evaluatePolicy(ctx, manifest, fleetPolicy) {
    ...
    if (decl === undefined) {
        ...
        const depthCheck = { allowed: false, minRequired: 5 };
    ...
    const tierEntries = manifest.tierAccess[ctx.tier] ?? [];
    const tierGranted = tierEntries.some((entry) => matchesTierEntry(ctx.capability, entry));
 ...
 */
function findCapabilityDecl(capability, decls) {
 ...
 */
function matchesResource(capability, resourcePattern) {
 ...
 */
function matchesTierEntry(capability, entry) {
    if (entry === capability)
        return true;
    if (entry.includes('*')) {
 ...
 */
function evaluateFleetFreezes(ctx, freezes) {
    for (const fz of freezes) {
        if (!matchesGlob(ctx.capability, fz.capabilityPattern))
            continue;
        if (!fz.tierScope.includes(ctx.tier))
            continue;
        if (fz.until !== undefined && fz.until <= ctx.ts)
            continue;
