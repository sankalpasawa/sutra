...
# measurement-logger.sh:31). Sid comes from the PostToolUse stdin JSON
# already captured in $INPUT. Session dir first via marker-lib;
...
if [ -f "$_MARKER_LIB" ]; then
  ...
  if command -v sutra_sid_from_stdin >/dev/null 2>&1; then
    sutra_sid_from_stdin "$INPUT" 2>/dev/null || true
  fi
fi
_b2_marker_path() {
