      ...
      if [ -z "$_MARKER_FILE" ]; then
        {
          ...
          echo "  Then (new format) — use the Write tool, NOT touch/redirect (Claude Code blocks Bash writes to .claude/):"
          echo "    Write file_path=.claude/depth-registered content=\"DEPTH=N TASK=<slug> TS=<unix>\""
          echo ""
        } >&2
        mkdir -p "$REPO_ROOT/.enforcement" 2>/dev/null
        ...
        if [ -z "$_PARSED_DEPTH" ]; then
          {
            ...
            echo "  Fix — use the Write tool, NOT touch/redirect (Claude Code blocks Bash writes to .claude/):"
            echo "    Write file_path=.claude/depth-registered content=\"DEPTH=5 TASK=my-task TS=<unix>\""
            echo ""
          } >&2
          mkdir -p "$REPO_ROOT/.enforcement" 2>/dev/null
          ...
          if [ "$_needs_5" = "1" ] && [ "$_DEPTH_VALUE" -lt 5 ]; then
            {
              ...
              echo "  Fix — use the Write tool, NOT touch/redirect (Claude Code blocks Bash writes to .claude/):"
              echo "    Write file_path=.claude/depth-registered content=\"DEPTH=5 TASK=<slug> TS=<unix>\""
              echo ""
            } >&2
            mkdir -p "$REPO_ROOT/.enforcement" 2>/dev/null
...
# Metric: hook_block_misses_with_visible_reason_pct.
# Formula: count(lines in .enforcement/routing-misses.log where corresponding hook-error in Claude Code showed BLOCKED message) / count(total miss lines). Target ≥ 95%.
# Data source: .enforcement/routing-misses.log + Claude Code session transcript review.
# Null handling: if no misses in window, report N/A (not 0).
#
# ### 2. Adoption mechanism
# Single file. Registered in .claude/settings.json PreToolUse hook. Active immediately on session restart. No downstream cascade — dispatcher is local to asawa-holding (npm/plugin packages copy from this file).
#
# ### 3. Monitoring / escalation
# Who: CEO of Asawa (self-monitored). Cadence: each session (noticed in-the-moment when blocked). Warn threshold: 3 silent blocks in one session. Breach threshold: 5 silent blocks. Escalation (initial, 2026-04-21): wrap brace-redirect around all six BLOCKED sections — DONE. Next escalation if silent blocks persist: investigate whether Claude Code's hook-error surface captures stderr at all for a given matcher; consider switching to `exec 2>&1` with explicit fd juggling.
#
# ### 4. Iteration trigger
# Revise when: new BLOCKED message type introduced without >&2 wrap, OR Claude Code hook-error surface changes behavior around stdout/stderr, OR observed silent-block rate >5% post-fix.
#
# ### 5. DRI
# Role: CEO of Asawa. Durable — this file is core governance, not handed off.
#
# ### 6. Decommission criteria
# Retire the brace-wrap pattern if Claude Code starts surfacing hook stdout directly, making stderr redirect unnecessary. Replace with simpler `echo` if/when that becomes true.
