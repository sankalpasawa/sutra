#!/usr/bin/env bash
#
# BUILD-LAYER: L1 PROMOTED TO L0 (2026-04-23)
#   Plugin L0 copy: sutra/marketplace/plugin/lib/override-audit.sh
#   Retention: this holding copy retained until 2026-05-23 (30d stability review).
#   Promotion session: Phase 1 Batch B restructure (codex session 019dba74).
#
...
# Metric: hook fire count + block/allow counts from hook-log.jsonl.
# Null handling: 0/0 tolerated when no triggering events.
#
# ### 2. Adoption mechanism
# Shipped via sutra plugin v1.12.2+; per-client opt-in via enabled_hooks.
# Holding L1 parallel during 30d stability review.
#
# ### 3. Monitoring / escalation
# Asawa Analytics scans hook-log every 3h. Warn: anomalous block rate.
# Breach: hook fails to fire on expected trigger.
#
# ### 4. Iteration trigger
# New matcher type; enforcement semantics change; D32 hot-reload lands.
#
# ### 5. DRI
# Sutra-OS (plugin L0). Asawa CEO for holding-copy retention.
#
# ### 6. Decommission criteria
# 2026-05-23 — delete holding L1 copy after stability review.
# ═══════════════════════════════════════════════════════════════════════════
