...
# plugin port 2026-08-26 (atom a-db25e6c4-10): registry = the repo's own
# os/method-registry.jsonl when present, else the plugin-shipped one.
PLUGIN_OS="$(cd "$(dirname "$0")/../os" 2>/dev/null && pwd)"
...
[ -f "$REGISTRY" ] || REGISTRY="$PLUGIN_OS/method-registry.jsonl"
