...
STANDARDIZED LAYOUT (founder 2026-07-30, v2): TWO LEVELS OF DEPTH, an INDEX on
the left, and a ONE-LINE DESCRIPTION per department (read `description` from
the domain files — never invent one; blank if absent).

CLI form: left-aligned index column (D1, D1.1, ...), name, then the
description in a second indented line; children grouped under their parent;
counts/bars only when work-unit data exists. Web form: generated by
`sutra/website/generate-departments.py` (left index rail, clickable anchored
sections, dotted-border child blocks) — regenerate + push to republish; never
hand-edit departments.html.
