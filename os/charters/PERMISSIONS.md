...
| 6 | **Remote / shared-state mutations** | leading token: `ssh`, `scp`, `rsync`, `aws`, `gcloud`, `vercel`, `supabase`, `doctl`, `fly`, `heroku`, `kubectl`, `helm`, `ansible`, `terraform`, `pulumi`, `render`, `railway`, `netlify`, `psql`, `mysql`, `mongo`, `mongosh`, `redis-cli`, `sqlite3`, `duckdb`; OR regex: `(npm\|yarn\|pnpm\|bun) publish`, `docker (push\|login)`, `(pip\|twine\|poetry) (upload\|publish)`. **`gh` is subcommand-aware (v2.6.1+):** read-only actions (`list`, `view`, `status`, `diff`, `checks`, `search *`, `auth status`, `repo clone`, `run watch`, `gh api` with default/GET method) auto-approve; mutations (`create`, `edit`, `delete`, `close`/`reopen`, `merge`, `comment`, `review`, `auth login`/`logout`, `secret set`/`delete`, `workflow run`/`disable`, `run cancel`/`rerun`, `codespace create`/`delete`/`ssh`, `extension install`, `ruleset create`, `project create`/`item-add`, `label create`/`delete`, `gh api -X POST\|PUT\|DELETE\|PATCH`) prompt. Mirrors `is_git_mutation` model. |
